...
print(c("final lc=",Lc,"final lbar=",LbarOut))
  ...
  if (sum(NumNans)<length(NumNans))
  {
   ...
   pdf(file=paste(FigureFolder,Fishes,' LBAR FvM Boxplots.pdf',sep=''))
   boxplot((MCDetails$FishingMortality/M)~MCDetails$Year,frame=F,xlab='Year',ylab='FvM',notch=F,outline=F,width=SampleSize) 

    
     dev.off()
    
    pdf(file=paste(FigureFolder,Fishes,' LBAR Fishing Mortality Boxplots.pdf',sep=''))
    boxplot((MCDetails$FishingMortality)~MCDetails$Year,frame=F,xlab='Year',ylab='F',notch=F,outline=F,width=SampleSize)
    dev.off()

    pdf(file=paste(FigureFolder,Fishes,' LBAR Total Mortality Boxplots.pdf',sep=''))
   boxplot((MCDetails$TotalMortality)~MCDetails$Year,frame=F,xlab='Year',ylab='Z',notch=F,outline=F, width=SampleSize)
    dev.off()

    pdf(file=paste(FigureFolder,Fishes,' LBAR Mean Length Boxplots.pdf',sep=''))
...
#figure<- ggarrange(mean_length,Total_M,F_boxplot,FvM_boxplot)

#annotate_figure(figure,
 #               top = text_grob(paste0("Species =", Species), color = "black", face = "bold", size = 14)
